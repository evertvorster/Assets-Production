...
vec4 atmosphericScatter(vec4 dif, float fNDotV, float fNDotL, float fVDotL, vec3 fvGShadow, vec3 fvAShadow)
{
   ...
   vec3  lscatter    = gl_LightSource[0].diffuse.rgb 
                       ...
                       * (fMinScatterFactor+min(fMaxScatterFactor-fMinScatterFactor,vdepth));
   
   vec4 rv;
   rv.rgb = regamma( (dif.rgb*labsorption*fvGShadow)*vabsorption 
...
{      
   ...
   vec3 fvAShadow          = lerp( vec3(1.0), fvShadowColor.rgb, fGAShadow.y );
